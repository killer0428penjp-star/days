    ...
    try {
      ...
      allNews = data.map(item => {
        ...
        return {
          ...
          title: item.title,
          link: item.link,
          rawDate: isNaN(d) ? new Date() : d,
          date: isNaN(d) ? "NEW" : `${d.getMonth()+1}/${d.getDate()} ${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`,
          img: `https://picsum.photos/seed/${encodeURIComponent(item.title)}/640/360`,
          count: Math.floor(Math.random() * 5000)
        };
      });
      renderFeed();
    } catch (e) {
      document.getElementById('feed').innerText = "ERROR";
    }
  }
    ...
    list.forEach((item, idx) => {
      ...
      card.innerHTML = `
        ...
        <div class="card-body">
          ...
          <div id="${cId}" class="news-content">読み込み中...</div>
          <div class="meta">
            <div style="display:flex; gap:15px; align-items:center;">
              <button class="btn-like ${likedItems.includes(item.id)?'liked':''}" onclick="toggleLike('${item.id.replace(/'/g, "\\'")}', this)">❤</button>
              <button class="btn-read" onclick="toggleNewsContent('${item.link}', '${cId}', this)">OPEN</button>
            </div>
            <span>${item.date} ${currentNewsTab==='popular'?'🔥'+item.count:''}</span>
          </div>
        </div>`;
  ...
  window.toggleNewsContent = async function(url, cId, btn) {
    ...
    if (card.classList.contains('open')) {
      ...
      if (!contentEl.getAttribute('data-loaded')) {
        contentEl.innerHTML = "読み込み中...";
        const res  = await fetch(`${GAS_URL}?url=${encodeURIComponent(url)}`);
        const text = await res.text();
        const formatted = text
          .split("\n\n")
          .map(p => `<p style="margin:0 0 12px; line-height:1.8;">${p}</p>`)
          .join("");
        contentEl.innerHTML = `
          <div style="font-size:14px; color:var(--text-main);">${formatted}</div>
          <a href="${url}" target="_blank" class="full-link">LINK TO SITE ↗</a>
        `;
