import { db, doc, getDoc, setDoc, updateDoc, collection, query, orderBy, limit, onSnapshot, arrayUnion, addDoc, serverTimestamp, deleteDoc, getDocs } from "./firebase-config.js";
...
window.execDeleteAccount = async () => { if(!confirm("本当に削除しますか？")) return; const targetId = me.id; await deleteDoc(doc(db, "users_v11", targetId)); removeFromHistory(targetId); np_logout(); };
...
function loadRooms() {
    onSnapshot(query(collection(db, "rooms_v11"), orderBy("createdAt", "desc")), (snap) => {
        ...
        snap.forEach(ds => { const r = ds.data(); if (r.members && !r.members.includes(me.id) && r.owner !== me.id) return; const memberCount = r.members ? r.members.length : 0; const tr = document.createElement('tr'); tr.className = 'data-row'; tr.innerHTML = ` <td style="width:50px" onclick="openChat('${ds.id}', '${r.name.replace(/'/g, "\\'")}')"><img src="${r.img || DEFAULT_IMG}" class="icon-cell"></td> <td style="text-align:left" onclick="openChat('${ds.id}', '${r.name.replace(/'/g, "\\'")}')"> <b>${r.name}</b> <span style="color:#888; font-size:12px;">(${memberCount})</span> </td> <td style="text-align:right; white-space:nowrap;"> ${(r.owner === me.id) ? ` <button class="btn-sub btn-edit" onclick="openRoomModal('${ds.id}','${r.name.replace(/'/g, "\\'")}','${r.img}')">編集</button> <button class="btn-sub btn-del" onclick="deleteRoom('${ds.id}')">削除</button> ` : ''} </td>`; list.appendChild(tr); });
    });
}
...
function loadFriends() { onSnapshot(collection(db, "users_v11"), (snap) => { const list = document.getElementById('friend-list-body'); list.innerHTML = ""; let userCount = 0; snap.forEach(ds => { const u = ds.data(); if(u.id === me.id) return; userCount++; const tr = document.createElement('tr'); tr.className = 'data-row'; const iconUrl = (u.icon && u.icon.val) ? u.icon.val : DEFAULT_IMG; const userDataJson = JSON.stringify(u).replace(/'/g, "\\'"); tr.innerHTML = ` <td style="width:50px"> <img src="${iconUrl}" class="icon-cell friend-icon" style="cursor:pointer" onclick='viewUserProfile(${userDataJson})'> </td> <td style="text-align:left"> <b>${u.name}</b><br><small style="color:#aaa">ID: ${u.id}</small> </td> <td></td>`; list.appendChild(tr); }); if(userCount === 0) list.innerHTML = "<tr><td colspan='2' style='text-align:center; color:#888; padding:20px;'>他のユーザーはいません</td></tr>"; }); }
...
document.getElementById('signup-exec-btn').onclick = async () => { const id = document.getElementById('auth-id').value.trim(), pw = document.getElementById('auth-pw').value.trim(), name = document.getElementById('signup-name').value.trim() || id; if(!id || !pw) return; const check = await getDoc(doc(db, "users_v11", id)); if(check.exists()) { document.getElementById('auth-err').innerText = "使用済みIDです"; return; } await setDoc(doc(db, "users_v11", id), { id, pw, name, icon: {val: DEFAULT_IMG}, createdAt: serverTimestamp() }); login(id, pw); };
document.getElementById('toggle-signup').onclick = () => { document.getElementById('signup-extra').classList.toggle('hidden'); document.getElementById('login-btn').classList.toggle('hidden'); document.getElementById('signup-exec-btn').classList.toggle('hidden'); };
document.getElementById('my-profile-btn-wrap').onclick = () => { document.getElementById('profile-modal').classList.remove('hidden'); document.getElementById('profile-edit-preview').src = me.icon.val || DEFAULT_IMG; document.getElementById('edit-profile-name').innerText = me.name; document.getElementById('edit-profile-id').innerText = "ID: " + me.id; document.getElementById('edit-birthday').value = me.birthday || ""; document.getElementById('edit-bio').value = me.bio || ""; };
document.getElementById('room-save-btn').onclick = async () => { const name = document.getElementById('room-name-input').value.trim(); if(!name) return; const selectedIds = Array.from(document.querySelectorAll('.invite-check:checked')).map(el => el.value); selectedIds.push(me.id); const roomData = { name, img: document.getElementById('room-preview').src, owner: me.id, members: selectedIds, createdAt: serverTimestamp() }; if(currentEditRoomId) { await updateDoc(doc(db, "rooms_v11", currentEditRoomId), { name, img: roomData.img, members: selectedIds }); } else { await addDoc(collection(db, "rooms_v11"), roomData); } closeModal('room-modal'); };
document.getElementById('profile-save-btn').onclick = async () => { const birthday = document.getElementById('edit-birthday').value, bio = document.getElementById('edit-bio').value.trim(), newIcon = document.getElementById('profile-edit-preview').src; await updateDoc(doc(db, "users_v11", me.id), { "icon.val": newIcon, birthday, bio }); me.icon.val = newIcon; me.birthday = birthday; me.bio = bio; document.getElementById('my-profile-btn').src = newIcon; alert("更新しました"); closeModal('profile-modal'); };
...
document.getElementById('room-file-input').onchange = (e) => { const f=e.target.files[0]; if(!f)return; const r=new FileReader(); r.onload=(v)=>document.getElementById('room-preview').src=v.target.result; r.readAsDataURL(f); };
document.getElementById('profile-file-input').onchange = (e) => { const f=e.target.files[0]; if(!f)return; const r=new FileReader(); r.onload=(v)=>document.getElementById('profile-edit-preview').src=v.target.result; r.readAsDataURL(f); };